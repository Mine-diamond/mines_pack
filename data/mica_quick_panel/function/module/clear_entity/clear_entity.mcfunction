...
execute if data storage mqp:clear_entity {kill_type:0} store result score #temp mqp.temp run kill @e[type=!minecraft:player,type=!minecraft:marker]
execute if data storage mqp:clear_entity {kill_type:1} store result score #temp mqp.temp run kill @e[type=!minecraft:player,type=!minecraft:item,type=!minecraft:marker]
execute if data storage mqp:clear_entity {kill_type:2} store result score #temp mqp.temp run kill @e[type=minecraft:item]
execute unless score @s mqp.core.message_feedback matches 0 run tellraw @s ["","[快捷功能]","清除了",{"score":{"name":"#temp","objective":"mqp.temp"},"color":"yellow"},"个实体"]
