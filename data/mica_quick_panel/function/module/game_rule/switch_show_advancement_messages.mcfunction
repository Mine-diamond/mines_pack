...
execute if score #show_advancement_messages mqp.game_rule.game_rule_state matches 0 as @a unless score @s mqp.core.message_feedback matches 0 run tellraw @s ["[快捷功能]",{"text": "已将§e[进度消息公告]§f调整为§btrue"}]
execute if score #show_advancement_messages mqp.game_rule.game_rule_state matches 1 as @a unless score @s mqp.core.message_feedback matches 0 run tellraw @s ["[快捷功能]",{"text": "已将§e[进度消息公告]§f调整为§cfalse"}]
