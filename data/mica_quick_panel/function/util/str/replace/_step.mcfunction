...
$execute if data storage mqp:util.str.replace {char:"$(from)"} run function mica_quick_panel:util/str/replace/_append_to with storage mqp:util.str.replace

# 如果当前字符不等于 from，则追加原字符 char
$execute unless data storage mqp:util.str.replace {char:"$(from)"} run function mica_quick_panel:util/str/replace/_append_char with storage mqp:util.str.replace
