...
execute if data storage mines_pack:projection_block_result_success {success:1} run tellraw @s [{"text":"你与前方方块的距离是"},{"storage":"mines_pack:projection_block_result_distant",nbt:"distant"}]
execute unless data storage mines_pack:projection_block_result_success {success:1} run tellraw @s [{"text":"前方没有方块"}]